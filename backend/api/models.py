...
class Sesion(models.Model):
    ...
    config_tiempos = models.JSONField(null=True, blank=True)
    creado_en = models.DateTimeField(auto_now_add=True)
    
    # --- CAMPOS NUEVOS PARA MULTIJUGADOR ---
    fase_actual = models.IntegerField(default=0)  # 0: Lobby, 1: Fase 1, etc.
    esta_pausada = models.BooleanField(default=False)
    # ---------------------------------------
    # NUEVO PARA FASE 4 GLOBAL
    sub_fase = models.CharField(max_length=20, default='', blank=True)
    ganador_ruleta = models.CharField(max_length=120, null=True, blank=True)
    equipo_presentando = models.CharField(max_length=120, null=True, blank=True)
    ...
    def __str__(self):
        return f"Sesión {self.codigo_acceso} - Fase {self.fase_actual}"
...
class Equipo(models.Model):
    ...
    # --- CAMPOS NUEVOS PARA MULTIJUGADOR ---
    termino_fase_actual = models.BooleanField(default=False)
    ultima_conexion = models.DateTimeField(auto_now=True)
    # ---------------------------------------

    # NUEVO PARA FASE 4 GLOBAL
    ya_presento_pitch = models.BooleanField(default=False)
