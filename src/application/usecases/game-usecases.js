...
export class GameUseCases {
  ...
  async finishPhase(body) { const codigo = codeFrom(body); const nombre = teamName(body); const session = await this.repository.requireSession(codigo); await this.repository.requireTeam(codigo, nombre); await this.repository.updateTeam(codigo, nombre, { terminoFaseActual: true }); const teams = await this.repository.listTeams(codigo); if (teams.length && teams.every((team) => team.termino_fase_actual)) { const stage = session.faseActual + 1; await this.repository.updateSession(codigo, { faseActual: stage, ...(stage >= 5 ? { estado: SESSION_STATUS.FINISHED, fechaFin: now() } : {}) }); await this.repository.resetTeams(codigo, { terminoFaseActual: false }); } return { status: 'ok' }; }
